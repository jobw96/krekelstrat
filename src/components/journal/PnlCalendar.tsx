    ...
    <section className="card-surface flex flex-col gap-3 p-5">
      ...
      <div className="grid grid-cols-7 gap-1.5">
        {["mon", "tue", "wed", "thu", "fri", "sat", "sun"].map((d) => (
          <span
            key={d}
            className="pb-1 text-center text-[10px] uppercase tracking-[0.08em] text-[#7A828D]"
          >
            {d}
          </span>
        ))}
        {cells.map((c) => {
          ...
          const color = positive ? WIN_GREEN : negative ? LOSS_RED : "#7A828D";
          return (
            <button
              key={key}
            ...
            >
              <span className="font-mono text-[11px] text-[#9AA1AC]">{c.day}</span>
              {dayTrades.length > 0 && (
                <span className="flex flex-col">
                  <span className="font-mono text-[12px] tabular" style={{ color, fontWeight: 560 }}>
                    {money(pnl)}
                  </span>
                  <span className="text-[10px] text-[#7A828D]">
